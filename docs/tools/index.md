...
### `sessions_list` / `sessions_history` / `sessions_send` / `sessions_spawn` / `sessions_wait` / `sessions_abort`
...
- `sessions_wait`: `runId`, `timeoutSeconds?`, `includeMessages?`, `messageLimit?`
- `sessions_abort`: `runId`, `sessionKey?`, `cleanup?`, `deleteTranscript?`
...
- `sessions_wait` checks a sub-agent run by `runId` and can return recent messages.
- `sessions_abort` aborts an in-flight run and can optionally delete the child session.
